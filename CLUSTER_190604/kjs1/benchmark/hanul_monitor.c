...
/*
		...
		system("sh /home/dcslab/benchmark/write_2.sh 1000");
		while(1) {
			...
			if(breakpoint){
				...
				if(fgets(space, sizeof(space)-1, fp) != NULL){
					...
					if(toggle){	//nvme is full
						system("sh /home/dcslab/benchmark/write_2.sh 30");
						printf("%s\n", "sh /home/dcslab/benchmark/write_2.sh 30");
						reply = redisCommand(c, "SET %s %d", "MIGRATE", 1);
						freeReplyObject(reply);
						...
						system("sh /home/dcslab/benchmark/write_2.sh 1000");
						printf("%s\n", "sh /home/dcslab/benchmark/write_2.sh 1000");
...
*/
		...
		while(1) {
			...
			fp2 = popen("iostat -d nvme1n1 1 2 | tail -n 2 | awk '{print $4}'", "r");
			if(fgets(perf, sizeof(perf)-1, fp2) != NULL) {
				fperf = atof(perf);
